...
create table if not exists events (
  ...
  visibility  text not null default 'public' check (visibility in ('public', 'connection_required', 'connections_only', 'unlisted')),
...
-- ---------- Official Yolink Bot ----------
-- This system profile is excluded from Discover. Every member is matched with
-- it automatically and staff can reply manually through the Bot RPC below.
do $$
declare
  v_bot uuid;
begin
  select value::uuid into v_bot from app_config where key = 'yolink_bot_profile_id';
  if v_bot is null or not exists (select 1 from profiles where id = v_bot) then
...
begin
  ...
  if v_bot is not null then
    ...
    where not exists (
      ...
      where match_id = v_match and sender_id = v_bot
        and body = '欢迎来到 Yolink！需要帮助可以随时问我。'
    );
  end if;
...
declare
  ...
  v_message messages;
begin
  select * into v_match from matches where id = p_match_id and (user_a = v_me or user_b = v_me);
  ...
  if not (
    ...
    or (v_event.visibility = 'connections_only' and exists (select 1 from matches where (user_a = v_me and user_b = v_event.creator_id) or (user_b = v_me and user_a = v_event.creator_id)))
    or (v_event.visibility = 'unlisted' and exists (select 1 from messages sent join matches m on m.id = sent.match_id where sent.event_id = v_event.id and (m.user_a = v_me or m.user_b = v_me)))
  ) then raise exception 'EVENT_NOT_VISIBLE'; end if;
  if v_event.visibility = 'connections_only' and v_other <> v_event.creator_id
...
create or replace function create_event(
  p_code text, p_title text, p_description text, p_starts_at timestamptz, p_location text, p_online_url text, p_visibility text, p_industries text, p_max_participants int
)
returns json
language plpgsql
security definer
...
declare
  ...
  v_visibility text;
begin
  select max_participants, starts_at, creator_id, visibility into v_capacity, v_starts_at, v_creator, v_visibility from events where id = p_event_id for update;
  if not found then
  ...
  if v_visibility in ('connection_required', 'connections_only') and v_profile <> v_creator
    and not exists (select 1 from matches where (user_a = v_profile and user_b = v_creator) or (user_b = v_profile and user_a = v_creator)) then
    raise exception 'EVENT_CONNECTION_REQUIRED';
  end if;
...
begin
  ...
  update events set title = trim(p_title), description = nullif(trim(p_description), ''),
    starts_at = p_starts_at, location = trim(p_location), online_url = nullif(trim(p_online_url), ''), visibility = coalesce(nullif(trim(p_visibility), ''), 'public'), industries = trim(p_industries),
...
-- Staff-only: reply from the official Bot in the conversation containing a
-- member's message. p_message_id is the id shown in the messages table.
...
declare
  ...
  v_message messages;
begin
  if not exists (select 1 from app_config where key = 'admin_passcode' and value = p_admin_pass) then
    raise exception 'BAD_ADMIN_PASSCODE';
  end if;
  ...
  join matches m on m.id = incoming.match_id
  where incoming.id = p_message_id and (m.user_a = v_bot or m.user_b = v_bot);
  if v_match is null then
    raise exception 'BOT_CONVERSATION_NOT_FOUND';
  end if;
  insert into messages (match_id, sender_id, body)
  values (v_match, v_bot, trim(p_body))
  returning * into v_message;
  return row_to_json(v_message);
end;
$$;

-- Return only the events a viewer may see. Unlisted events require their shared link.
...
begin
  return json_build_object(
    'events', coalesce((select json_agg(row_to_json(e) order by e.starts_at) from events e where exists (
      select 1 from messages invite join matches m on m.id = invite.match_id
      where invite.event_id = e.id and (m.user_a = v_me or m.user_b = v_me)
    )), '[]'::json),
    'participants', coalesce((select json_agg(row_to_json(ep)) from event_participants ep where exists (
      select 1 from messages invite join matches m on m.id = invite.match_id
